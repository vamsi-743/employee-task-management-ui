...
import {
  addEmployee,
  emsCreateEmployee,
  emsGetAllDepartments,
  emsGetAllDesignations,
  emsGetEmployeeIdList,
  getAllDepartments,
  getAllDesignations,
  getEmployeeIds,
} from '@/http/api'
...
const CreateEmployee = () => {
  ...
  const {
    ...
    queryKey: ['ems_departments'],
    queryFn: () => emsGetAllDepartments(payload),
  })
  ...
  const {
    ...
    queryKey: ['ems_employeeIds'],
    queryFn: emsGetEmployeeIdList,
  })
  ...
  const form = useForm<EmsEmployeeFormValues>({
    resolver: zodResolver(emsEmployeeSchema),
    defaultValues,
    mode: 'onChange',
  })
  ...
  const processForm: SubmitHandler<EmsEmployeeFormValues> = (data) => {
    console.log('data ==create employee>', { ...employeeData, ...data })
    const newData = {
      ...employeeData,
      ...data,
      EMS_employee_id: data.employee_id,
  ...
  type FieldName = keyof EmsEmployeeFormValues
  ...
  const next = async () => {
    ...
    const employeeExists = employeeIdsList.some(
      (employee: any) => employee.EMS_employee_id === employeeId
    )
    if (employeeExists) {
